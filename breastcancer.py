...
#diffusion map
sc.tl.diffmap(data_trans)

sc.tl.tsne(data_trans,n_pcs=20)

# first component
first=data_trans.obsm['X_diffmap'][:,0]
data_trans.obs['first']=first
sc.pl.tsne(data_trans,color=['first'])

#second component
second=data_trans.obsm['X_diffmap'][:,1]
data_trans.obs['second']=second
sc.pl.tsne(data_trans,color=['second'])

#third component
third=data_trans.obsm['X_diffmap'][:,2]
data_trans.obs['third']=third
sc.pl.tsne(data_trans,color=['third'])

sc.pp.neighbors(data_trans, n_neighbors=40, use_rep='X_diffmap')
sc.tl.paga(data_trans, groups='leiden')
sc.pl.paga_compare(data_trans)

data_trans.uns['iroot'] = np.flatnonzero(data_trans.obs['leiden']  == 'Lymphocytes and Tumor1')[0]
sc.tl.dpt(data_trans)

sc.tl.draw_graph(data_trans, init_pos='paga')
sc.pl.draw_graph(data_trans, color=['leiden', 'dpt_pseudotime'], legend_loc='on data')


right=pd.read_csv('/home/xixi949592/GTEx_small_gene_tpm_new.txt',sep='\t')

right_new=right.drop(columns=['Description']).set_index('Name')
right_new_1=right_new.mask(right_new<1,0)
right_new_1.to_csv(r'/home/xixi949592/normal_tissue.txt',header=True,index=True,sep='\t')
normal=sc.read_text('/home/xixi949592/normal_tissue.txt').transpose()

# normal tissue filtering
sc.pp.filter_cells(normal, min_genes=1000)
sc.pp.filter_genes(normal, min_cells=600)
...
normal = normal[normal.obs.n_genes_by_counts < 21000, :]
#normal = normal[normal.obs.pct_counts_mt < 20, :]
normal
#data.obs['total_counts']


normaldf=normal.to_df().transpose()
result = pd.merge(cancerdf, normaldf, left_index=True, right_index=True, how='outer')
result_inner=pd.merge(cancerdf, normaldf, left_index=True, right_index=True, how='inner')
result_new=result.fillna(0)
result_new_inner=result_inner.fillna(0)
result_new.to_csv(r'/home/xixi949592/TPM_merge.txt',header=True,index=True,sep='\t')
result_new_inner.to_csv(r'/home/xixi949592/TPM_merge_inner.txt',header=True,index=True,sep='\t')

#pd.read_csv('/home/xixi949592/TPM_merge.txt',sep='\t',usecols=[0])
# merge=pd.read_csv('/home/xixi949592/TPM_merge.txt',sep='\t', usecols=[0,1,2])
# merge.fillna(0)

# read in merge data
adata=sc.read_text('/home/xixi949592/TPM_merge.txt').transpose()

#read in inner merged data
adata_inner=sc.read_text('/home/xixi949592/TPM_merge_inner.txt').transpose()
adata_inner
...
# PCA inner data
sc.tl.pca(adata_inner, svd_solver='arpack')
sc.pl.pca(adata_inner)
...
sc.tl.tsne(adata_inner,n_pcs=20)
sc.pl.tsne(adata_inner)
sc.pp.neighbors(adata_inner, n_neighbors=50)
sc.tl.leiden(adata_inner,resolution=1)
sc.pl.tsne(adata_inner, color=['leiden'],legend_loc='on data')

adata.obs['leiden']

adata_inner.obs['leiden']

for i in range(35):
    print(i,adata.obs[adata.obs['leiden']==str(i)].shape)

for i in range(33):
    print(i,adata_inner.obs[adata_inner.obs['leiden']==str(i)].shape)

for i in range(35):
    if adata.obs[adata.obs['leiden']==str(i)].shape == adata.obs[adata.obs['leiden']==str(i)].filter(regex='GT',axis=0).shape:
        print(i, adata.obs[adata.obs['leiden']==str(i)].filter(regex='GT',axis=0).shape, 'True')
    else:
        print(i, 'False')

for i in range(33):
    if adata_inner.obs[adata_inner.obs['leiden']==str(i)].shape == adata_inner.obs[adata_inner.obs['leiden']==str(i)].filter(regex='GT',axis=0).shape:
        print(i, adata_inner.obs[adata_inner.obs['leiden']==str(i)].filter(regex='GT',axis=0).shape, 'True')
    else:
        print(i, 'False')
...
# first component
first=data.obsm['X_diffmap'][:,0]
data.obs['first']=first
sc.pl.tsne(data,color=['first'])

#second component
second=data.obsm['X_diffmap'][:,1]
data.obs['second']=second
sc.pl.tsne(data,color=['second'])

#third component
third=data.obsm['X_diffmap'][:,2]
data.obs['third']=third
sc.pl.tsne(data,color=['third'])



data=sc.read_text('/home/xixi949592/Breast_Cancer_singlecell_TPM_matrix.txt')

# df=data.to_df().transpose()
# BC01=df.filter(regex='^BC01',axis=1)
# BC02=df.filter(regex='^BC02',axis=1)
# BC03=df.filter(regex='^BC03',axis=1)
# BC03LN=df.filter(regex='^BC03LN',axis=1)
# BC04=df.filter(regex='^BC04',axis=1)
# BC05=df.filter(regex='^BC05',axis=1)
# BC06=df.filter(regex='^BC06',axis=1)
# BC07=df.filter(regex='^BC07',axis=1)
# BC07LN=df.filter(regex='^BC07LN',axis=1)
# BC08=df.filter(regex='^BC08',axis=1)
# BC09=df.filter(regex='^BC09',axis=1)
# BC10=df.filter(regex='^BC10',axis=1)
# BC11=df.filter(regex='^BC11',axis=1)
...
import numpy as np
np.array(BC01).flatten().shape

BC01_mtx = np.array(BC01)
...
sc.tl.umap(data)
sc.pl.umap(data, color=['BC01','BC02','BC03','BC04','BC05','BC06'])
